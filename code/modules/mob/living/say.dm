...
/mob/living/say(message, bubble_type, list/spans = list(), sanitize = TRUE, datum/language/language = null, ignore_spam = FALSE, forced = null, only_overhead, mob/direct_to_mob = null)
	...
	var/sigreturn = SEND_SIGNAL(src, COMSIG_MOB_SAY, args) // signal ffgneriugvnbiresgniuer
...
/*Optimisation as we don't use space
	...
	if(pressure < ONE_ATMOSPHERE*0.4) //Thin air, let's italicise the message
		spans |= SPAN_ITALICS
*/
	send_speech(message, message_range, src, bubble_type, spans, language, message_mode, only_overhead)
	/* if(succumbed)
		succumb()
...
/mob/living/Hear(message, atom/movable/speaker, datum/language/message_language, raw_message, radio_freq, list/spans, message_mode, atom/movable/source, only_overhead = FALSE, list/data)
...
/mob/Hear(
	...
	if(islist(data) && LAZYACCESS(data, "is_radio") && (data["ckey"] in GLOB.directory))
